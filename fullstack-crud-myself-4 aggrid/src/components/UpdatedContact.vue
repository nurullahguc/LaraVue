<template>
  ...
  <div class="container p-4">
    <div class="card">
      <div class="row p-2">
        <div class="col-md-6">
          <h2 class="text-center text-primary">Update Contact</h2>
...
const updateContact = () => {
  ...
  axios.post(base_url + "/update_contact/" + router.params.id, contact)
      .then(response => {
...
const getContact = () => {
  ...
  axios.get(base_url + "/get_contact/" + router.params.id)
      .then(response => {
        ...
        contact.contact_no = response.data.contact_no
      })
      .catch(error => {
        ToastMessage('error', error.message)
      })
